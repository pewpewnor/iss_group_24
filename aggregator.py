...
Bbox storage policy:
    - Support bboxes are computed from masks (InsDet) / largest non-white
      component (HOTS) and stored in the manifest as METADATA.  The
      training pipeline does NOT crop supports to bbox; supports are
      letterboxed to img_size.  Bboxes remain available for downstream
      analyses or future variants.
    - Query bboxes are the GT for localizer training.

Run:
    uv run python aggregator.py            # build (idempotent)
    uv run python aggregator.py --force    # rebuild from scratch
    uv run python aggregator.py --validate # validate existing manifest
"""
...
def collect_hots_instances() -> list[dict]:
    ...
    print(f"hots: {len(out)} instances")
    return out
...
def collect_insdet_instances() -> list[dict]:
    ...
    for obj_dir in sorted(d for d in objects_dir.iterdir() if d.is_dir()):
        ...
        for p in img_files:
            ...
            bb = _bbox_from_mask(mask) if mask.exists() else _bbox_from_image(p)
            if bb is None:
                continue
            support.append({"path": str(p), "bbox": bb})
        if len(support) < N_SUPPORT_MIN:
    ...
    print(f"insdet: {len(out)} instances")
    return out
...
def attach_scene_queries(instances: list[dict], scene_queries: list[dict]) -> list[dict]:
    ...
    print(f"scene attached: {matched}/{len(instances)} instances matched")
    return out
...
def stage_images(splits: dict[str, list[dict]]) -> None:
    ...
    for split_name, insts in splits.items():
        for inst in insts:
            ...
            for role, img_list in (("support", inst["support_images"]),
                ...
                for idx, img in enumerate(img_list, start=1):
                    ...
                    n_copied += 1
                    img["path"] = (Path(split_name) / role / inst_id / dst.name).as_posix()
...
def write_stats(splits: dict[str, list[dict]]) -> None:
    stats: dict = {"schema_version": SCHEMA_VERSION,
...
def validate(strict: bool = True) -> bool:
    p = OUT_DIR / "manifest.json"
    if not p.exists():
        print(f"validate: {p} missing")
        return False
    with open(p) as f:
        m = json.load(f)
    if m.get("schema_version") != SCHEMA_VERSION:
        print(f"validate: schema_version mismatch (got {m.get('schema_version')}, want {SCHEMA_VERSION})")
        if strict:
            return False
    n_checked = 0
    n_bad = 0
    for inst in m["instances"]:
        ...
        for role in ("support_images", "query_images"):
            for img in inst.get(role, []):
                pth = OUT_DIR / img["path"]
                if not pth.exists():
                    print(f"validate: missing {pth}")
                    n_bad += 1
                    continue
                bbox = img.get("bbox")
                if bbox is None or len(bbox) != 4:
                    print(f"validate: bad bbox in {pth}")
                    n_bad += 1
                    continue
                n_checked += 1
    print(f"validate: checked {n_checked} (path, bbox) pairs, {n_bad} bad")
    return n_bad == 0
...
def main(force: bool = False) -> None:
    if OUT_DIR.exists() and not force:
        if validate(strict=True):
            print(f"aggregator: dataset already staged at {OUT_DIR} and validates; skipping.")
            return
        print(f"aggregator: dataset at {OUT_DIR} did NOT validate; rebuilding.")
    if OUT_DIR.exists():
    ...
    print("validating freshly built manifest")
    if not validate(strict=True):
        print("aggregator: post-build validate FAILED")
        sys.exit(1)


if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument("--force", action="store_true", help="rebuild even if staged dataset exists and validates")
    parser.add_argument("--validate", action="store_true", help="only validate existing staged dataset")
    args = parser.parse_args()
    if args.validate:
        ok = validate(strict=True)
